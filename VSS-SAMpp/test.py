...
data_mean = 48.36285132049267
data_std = 70.92476936742324


def test_single_volume(image, label, net, classes, multimask_output, patch_size=[256, 256], test_save_path=None,
                       case=None):
    ...
    probability = np.concatenate((probability[0:1], probability[0:1], probability, probability[-1:], probability[-1:]),
                                 axis=0)
    ...
    for ind in range(image.shape[0]):
        ...
        with torch.no_grad():
            ...
            out_pred = torch.squeeze(out_pred, dim=0)
            out_pred = torch.permute(out_pred, (3, 1, 2, 0))
...
def inference(args, multimask_output, model, test_save_path=None):
    ...
    for data_fd in tqdm(data_fd_list):
        ...
        for image_file in image_file_list:
            ...
            with open(args.data_path + '/' + data_fd + '/masks/' + image_file.replace("2Dimage", "2Dmask"),
                      'rb') as file:
                mask_arr = pickle.load(file)

            image_arr = np.clip(image_arr, HU_min, HU_max)
            image_arr = (image_arr - HU_min) / (HU_max - HU_min)
            ...
            class_to_name = {1: 'Spleen', 2: 'Right Kidney', 3: 'Left Kidney', 4: 'Gall Bladder', 5: 'Esophagus', 6: 'Liver', 7: 'Stomach', 8: 'Aorta', 9: 'Bladder', 10: 'Prostate/Uterus'}
...
if __name__ == '__main__':
    ...
    parser.add_argument('--adapt_ckpt', type=str, default=None, help='The checkpoint after adaptation')
