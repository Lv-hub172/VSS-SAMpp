...
def organize_data():
    ...
    data_fd_list = [data_fd for data_fd in data_fd_list if
                    data_fd.endswith('.nii.gz') and 'segmentation' not in data_fd]
    data_fd_list.sort()
    ...
    for data_fd in data_fd_list:
        ...
        shutil.copy(base_dir + '/' + 'prostateD/' + data_fd,
                    save_pth + '/imagesTr/prostateD_' + patient_ID + '_0000.nii.gz')
    ...
    for data_pth in data_pth_all:
        ...
        for data_fd_indx, data_fd in enumerate(data_fd_list):
            ...
            img_arr = np.concatenate(
                (img_arr[:, :, 0:1], img_arr[:, :, 0:1], img_arr, img_arr[:, :, -1:], img_arr[:, :, -1:]), axis=-1)
            mask_arr = np.concatenate(
                (mask_arr[:, :, 0:1], mask_arr[:, :, 0:1], mask_arr, mask_arr[:, :, -1:], mask_arr[:, :, -1:]), axis=-1)
            ...
            for slice_indx in range(2, img_arr.shape[2] - 2):
                ...
                with open(save_pth + '/' + case_id + '/images' + '/2Dimage_' + '{:04d}'.format(slice_indx - 2) + '.pkl',
                          'wb') as file:
                    pickle.dump(slice_arr, file)

                with open(save_pth + '/' + case_id + '/masks' + '/2Dmask_' + '{:04d}'.format(slice_indx - 2) + '.pkl',
                          'wb') as file:
