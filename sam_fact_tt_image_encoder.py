...
class Fact_tt_Sam(nn.Module):
    ...
    def save_parameters(self, filename: str) -> None:
        ...
        for key, value in state_dict.items():
            if 'Decoder' in key:
                Decoder_tensors[key] = value
            if 'vmamba' in key:
                vmamba_tensors[key] = value
            if 'seg_decoder' in key:
                seg_decoder_tensors[key] = value
            if 'merge' in key:
                merge_tensors[key] = value
            if '.adapter_' in key:
        ...
        merged_dict = {**a_tensors, **b_tensors, **FacTu_tensors, **FacTv_tensors,  **Decoder_tensors, **adapter_tensor,**seg_decoder_tensors,**merge_tensors,**vmamba_tensors}
    ...
    def load_parameters(self, filename: str) -> None:
        ...
        # load Decoder
        Decoder_keys = [k for k in sam_keys if 'Decoder' in k ]
        Decoder_values = [state_dict[k] for k in Decoder_keys]
        Decoder_new_state_dict = {k: v for k, v in zip(Decoder_keys, Decoder_values)}
        sam_dict.update(Decoder_new_state_dict)

        # load merge
        merge_keys = [k for k in sam_keys if 'merge' in k ]
        merge_values = [state_dict[k] for k in merge_keys]
        merge_new_state_dict = {k: v for k, v in zip(merge_keys, merge_values)}
        sam_dict.update(merge_new_state_dict)

        # load vmamba
        vmamba_keys = [k for k in sam_keys if 'vmamba' in k ]
        vmamba_values = [state_dict[k] for k in vmamba_keys]
        vmamba_new_state_dict = {k: v for k, v in zip(vmamba_keys, vmamba_values)}
        sam_dict.update(vmamba_new_state_dict)

        # load seg_decoder
        seg_decoder_keys = [k for k in sam_keys if 'seg_decoder' in k]
        seg_decoder_values = [state_dict[k] for k in seg_decoder_keys]
        seg_decoder_new_state_dict = {k: v for k, v in zip(seg_decoder_keys, seg_decoder_values)}
        sam_dict.update(seg_decoder_new_state_dict)
